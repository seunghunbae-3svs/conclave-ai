...
### Added
- **`conclave init` auto-migration of managed workflow files
  (cli@0.13.18, H1 #1).** Pre-fix: re-running `conclave init` on an
  existing install was binary — either skipped every existing file
  (idempotent but staleness-blind) or required `--reconfigure` to
  overwrite (carpet-bomb that also blew away user customisations).
  Live RC: eventbadge#34 — manual PR was needed to migrate the
  v0.7-era inline `conclave-merge.yml` to the v0.13.17 thin wrapper
  even though the file had our managed-header marker.

  v0.13.16 detects "we own this file" via the `Generated by \`conclave init\``
  header marker. The status type is now four-valued:
  - **created**: file didn't exist → wrote fresh
  - **migrated**: file existed AND has our marker AND content
    differs from current template → overwrote (we own it)
  - **skipped-current**: matches current template byte-for-byte → no-op
  - **skipped-customised**: file exists but lacks our marker → user
    owns it, leave alone (`--reconfigure` overrides)

  `conclave init` output now uses 4 distinct verbs (`wrote`,
  `migrated`, `ok`, `keep`) so a casual scan tells the user what
  moved. `formatWorkflowStatusLine` is exported for testability.

  **Tests:** 6 new init-test cases cover the H1 #1 scenarios — H1
  case (managed + stale → migrated), idempotent re-run (matches →
  skipped-current), user-customised preservation (no marker → keep),
  `--force` always overwrites, `formatWorkflowStatusLine` 4-state
  shape. 481/481 cli tests pass.
...
### Fixed
- **Dedupe blockers by (file, line, message-prefix), not category
  (cli@0.13.6 / core).** Pre-fix `dedupeBlockersAcrossAgents` keyed on
  `category|file|message[:80]`. When 3 agents reported the same
  observable bug (stray `console.log` on `AddressSearch.jsx:2`) under
  three DIFFERENT category labels (regression / logging /
  code-quality), the dedupe key diverged and all 3 passed through.
  autofix generated 3 patches all targeting the same line, the second
  apply hit context mismatch (line was already gone), the loop
  stalled at cycle 1/3.

  Fix: drop `category`. New key = `file|line|message[:60]`. Line
  disambiguates same-file repeats; 60-char message prefix
  differentiates distinct bugs at the same line without locking out
  near-paraphrases from different agents. Live-caught on
  eventbadge#28 rework run 24958156176. Backfilled (873bf36).

## v0.13.5 — 2026-04-26

### Fixed
- **Autofix: dump patch + target-file snippet on apply conflict
  (cli@0.13.5).** Pre-fix the conflict reason was just
  `patch failed: <file>:<line>` with the patch tempfile already
  unlinked. The patch's head (1500 c) and the target file's first
  12 lines are now captured before the temp delete, so context
  mismatches (line endings / encoding / drift) are visible from CI
  logs alone. Live-needed on eventbadge#28 rework run 24957853174
  where the original diagnostic gave `patch failed: AddressSearch.jsx:1`
  with no way to see which patch line broke it. Backfilled (ecb21c6).

## v0.13.4 — 2026-04-26

### Fixed
- **Autofix: stage ONLY patched files, not all working-tree changes
  (cli@0.13.4).** Live-caught on eventbadge#25 commit 22a0b99: the
  autofix commit included `frontend_old/package-lock.json` (3018
  lines generated by the dependency-install step), which the next
  review flagged as a compatibility blocker, blocking loop closure.
  Root cause: autofix used `git add -A`. Now tracks exactly which
  files autofix patched (`appliedPaths` from the unified-diff applies
  + `appliedFiles` from special handlers) and `git add --` only those
  paths. Defense-in-depth fallback uses `git add -u` (tracked-only)
  when `filesToStage` is empty. Backfilled (53d11b9).

## v0.13.3 — 2026-04-26

### Fixed
- **Autofix: dump per-blocker status when 'no applicable patches'
  bails (cli@0.13.3).** Pre-fix: when all worker patches dropped
  (worker-error / conflict / secret-block), autofix logged a single
  line "all blockers skipped/conflict" with zero breakdown. Now
  dumps `[status] category @ file: reason` per blocker — same shape
  as `--dry-run`'s breakdown. Live-needed for eventbadge#25 e2e
  diagnosis. Backfilled (d439c05).

## v0.13.2 — 2026-04-26

### Fixed
- **Autofix: `--no-notify` flag + apply conflict diagnostics
  (cli@0.13.2).** Two issues caught during eventbadge#25 live E2E:
  1. Autofix's spawned `conclave review --pr N --json` (the
     verdict-fetcher used when `--verdict` isn't passed) was sending
     a fresh `notifyReview` to Telegram — duplicating the upstream
     review's notification. User-visible: "rework 0/3" appearing 3-4
     times in chat per cycle. Fix: new `--no-notify` flag that stubs
     the notifier list to `[]`; autofix's default spawnReview now
     passes it.
  2. Apply-conflict logging was a one-liner with no context. Now
     dumps the rejecting fix's file + reason tail to stderr.

  Both are caller-side (no protocol or DB changes). Backfilled
  (0357a9e).

## v0.13.1 — 2026-04-26

### Tooling (no source change)
- **Release-version bump only (cli@0.13.1).** First post-v0.13.0
  release run; no functional change. The lockstep release flow
  bumped every package's patch version even though no `packages/*`
  source had moved since v0.13.0. (One of the motivations for
  v0.13.12's per-package bumping.) Tagged at `ad7dfd2`.
